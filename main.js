...
$(function () {
    ...
    function VehicleList()      //EmailId
    {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                VList = result;
                if (result.length > 0) {
                    $('#LicenseNumberList').html('');
                    var options = '';
                    options += '<option value="Select">Select</option>';
                    for (var i = 0; i < result.length; i++) {
                        options += '<option value="' + result[i].LicenseNumber + '">' + result[i].LicenseNumber + '</option>';
                    }
                    $('#LicenseNumberList').append(options);
                    
                      
                               
                }
                else {
                    alert("No vehicle are registered on this email!!");
                }
            },
            error: function (errormessage) {
                alert(errormessage.responseText);
    ...
    $('#LicenseNumberList').on('change', function () {
        var selectedVehicle = $(this).children("option:selected").val();
        alert("You have selected the Vehicle - " + selectedVehicle);
        var licenseNumber = this.value;
        alert(this.value);
